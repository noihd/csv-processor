...
const mappings = {
  ...
  SIE_2: {
    column: 734,
    is_required: false,
    model_field: 'violent_misdemeanor',
    model_name: 'FirearmLaws',
    model_type: 'boolean',
    max_decimal: null,
    max_length: null,
  ...
  SIE_3: {
    column: 735,
    is_required: false,
    model_field: 'universal_background_check',
    model_name: 'FirearmLaws',
    model_type: 'boolean',
    max_decimal: null,
    max_length: null,
  ...
  SIE_5: {
    column: 737,
    is_required: false,
    model_field: 'count_of_three_laws',
    model_name: 'FirearmLaws',
    model_type: 'integer',
    max_decimal: null,
    max_length: 1,
  ...
  SIE_6: {
    column: 738,
    is_required: false,
    model_field: 'count_of_three_laws_weighted',
    model_name: 'FirearmLaws',
    model_type: 'float',
    max_decimal: 8,
    max_length: 9,
  ...
  SIE_9: {
    column: 741,
    is_required: false,
    model_field: 'gun_ownership_proxy_improved',
    model_name: 'FirearmLaws',
    model_type: 'float',
    max_decimal: 8,
    max_length: 11,
...
module.exports = () => {
   ...
   * Generate SQL File
   */
  const generateSQL = columns => {
    let sql = 'DROP TABLE IF EXISTS `pdp_raw_data`;\nCREATE TABLE `pdp_raw_data` ('

    columns.forEach(key => {
      let obj = mappings[key]
      let dec = obj.max_decimal
      let max = obj.max_length
      let required = obj.is_required ? "NOT NULL DEFAULT '0'" : 'DEFAULT NULL'
      let unsigned = obj.unsigned ? 'unsigned ' : ''
      let type = obj.model_type

      if (type === 'integer') {
        sql += `\n  \`${key}\` int(${max}) ${unsigned}${required},`
      } else if (type === 'float') {
        sql += `\n  \`${key}\` decimal(${max},${dec}) ${unsigned}${required},`
      } else if (type === 'double') {
        sql += `\n  \`${key}\` double(${max},${dec}) ${unsigned}${required},`
      } else if (type === 'float') {
        sql += `\n  \`${key}\` float(${max},${dec}) ${unsigned}${required},`
      } else if (type === 'boolean') {
        sql += `\n  \`${key}\` tinyint(1) ${required},`
      } else if (type === 'boolean') {
        sql += `\n  \`${key}\` tinyint(1) ${required},`
      } else if (type === 'string') {
        required = obj.is_required ? "NOT NULL DEFAULT ''" : 'DEFAULT NULL'
        sql += `\n  \`${key}\` varchar(${max}) ${required},`
      } else if (type === 'text') {
        required = obj.is_required ? "NOT NULL DEFAULT ''" : 'DEFAULT NULL'
        sql += `\n  \`${key}\` TEXT ${required},`
      } else if (type === 'enum') {
        // CSV does not actually use enum, so convert back to int for raw data
        sql += `\n  \`${key}\` int(3) ${required},`
      } else {
        // We skipped some columns that we don't need, but let's put them back in for raw data
        sql += `\n  \`${key}\` varchar(10) ${required},`
      }
    })
  ...
  const getEnumValue = (key, val) => {
    ...
    if (key.indexOf('LEM_9') > -1) {
      key = 'LEM_9'
    }

    if (typeof enumMappings[key] === 'undefined' || typeof enumMappings[key][val] === 'undefined') {
      // These keys are known to cause issues, and we can safely ignore them
      const ignoredKeys = ['XWK_23']

      if (val && val.length > 0 && ignoredKeys.indexOf(key) === -1) {
        console.error(`Invalid Enum Mapping ${originalKey} ${val}`)
      }
